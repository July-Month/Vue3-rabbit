...
const getDetail = async () => {
  const res = await getGoodDetailAPI({
    id: route.params.id
  })
...
const addCart = () => {
  if (skuObj.value.skuId) {
    // 选择了规格 触发action
    cartStore.addCart({
      id: detail.value.id, // 商品id
      name: detail.value.name, // 商品名称
      picture: detail.value.mainPictures[0], // 图片
      price: detail.value.price, // 最新价格
    ...
    <div class="container">
      <div class="bread-container">
        <el-breadcrumb separator=">">
          ...
          <!-- 
            错误原因：detail 一开始是{} {}.categories => 是undefined => undefined[1]
            解决办法：
            1. 可选链语法 ?.
            2. v-if 手动控制标签渲染时机,保证只有数据存在才渲染
          -->
          <el-breadcrumb-item
